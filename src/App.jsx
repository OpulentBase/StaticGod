    ...
    await Promise.all(
      outputs.map(async (out) => {
        try {
          ...
          const proxyUrl = `/api/proxy-image?url=${encodeURIComponent(out.url)}`;
          const res = await fetch(proxyUrl);
          if (!res.ok) throw new Error("Proxy error " + res.status);
          const blob = await res.blob();
          zip.file(`${out.path}/${out.name}`, blob);
        } catch (e) {
          console.error("[SG zip] failed to fetch", out.url, e.message);
        }
      })
    );
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .prompt-toolbar-left { display: flex; align-items: center; gap: 10px; }
  .prompt-count {
    font-size: 12px; font-weight: 700; color: var(--text);
    font-family: 'Barlow', sans-serif;
  }
  .prompt-count span { color: var(--accent); }
  .btn-reset {
    font-size: 11px; padding: 5px 12px; border-radius: 7px;
    border: 1px solid var(--border2); background: transparent;
    color: var(--muted2); cursor: pointer; font-family: 'Barlow', sans-serif;
  ...
  .prompt-card:hover { border-color: var(--accent); box-shadow: 0 4px 16px rgba(0,0,0,.25); }
  .prompt-card.expanded { grid-column: span 2; }
  ...
  .prompt-card-angle {
    ...
    font-family: 'Barlow', sans-serif; margin-bottom: 2px;
  }
  .prompt-card-demo {
    font-size: 11px; font-weight: 700; color: var(--text);
    font-family: 'Barlow', sans-serif; line-height: 1.3;
  }
  ...
  .prompt-card-remove:hover { border-color: var(--red); color: var(--red); background: rgba(240,80,80,.08); }

  .prompt-card-body {
    padding: 10px 12px; flex: 1; cursor: pointer;
  }
  .prompt-card-text {
  ...
  .prompt-expand-btn:hover { opacity: .7; }
  ...
  @media (max-width: 800px) {
    ...
    .prompt-card.expanded { grid-column: span 1; }
...
Generate exactly ${numAds} unique static ad prompts for this product. Each must attack a completely different psychological angle. Return only the JSON.`;
  ...
  const removePrompt = (si, pi) => {
    setSections(prev => {
      const updated = prev.map((sec, i) => {
        if (i !== si) return sec;
        return { ...sec, prompts: sec.prompts.filter((_, j) => j !== pi) };
      }).filter(sec => sec.prompts.length > 0);
      return updated;
    });
  };

  // Flatten sections into [{si, pi, title, angle, demo, prompt}]
  const flatPrompts = sections.flatMap((sec, si) => {
    const parts = sec.title.split("—").map(p => p.trim());
    const angle = parts[0] || sec.title;
    const demo = parts[1] || "";
    return sec.prompts.map((p, pi) => ({ si, pi, title: sec.title, angle, demo, prompt: p }));
  });
                ...
                <div>
                  ...
                  <div className="folder-preview">
                    <div className="folder-row">
                      ...
                      <span className="fname">Batch_{todayStr()}</span>
                      <span className="fmeta">{totalPrompts} imgs</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="tab-content">
                {flatPrompts.length === 0 ? (
                  <div className="empty">
                    ...
                    <p>Use AI Generate or upload an HTML template. Each prompt becomes one ad.</p>
                  </div>
                ) : (
                  <>
                    {/* Toolbar */}
                    <div className="prompt-toolbar">
                      <div className="prompt-toolbar-left">
                        <span className="prompt-count">
                          <span>{flatPrompts.length}</span> prompt{flatPrompts.length !== 1 ? "s" : ""} ready to generate
                        </span>
                        {!running && (
                          <button
                            className="btn-reset"
                            onClick={() => { setSections([]); setPromptStates({}); }}
                          >Clear all</button>
                        )}
                      </div>
                      <span style={{ fontSize: 10, color: "var(--muted)", fontFamily: "'JetBrains Mono',monospace" }}>
                        Click any card to expand · ✕ to remove
                      </span>
                    </div>

                    {/* Card Grid */}
                    <div className="prompt-grid">
                      {flatPrompts.map((item, idx) => {
                        const key = `${item.si}-${item.pi}`;
                        const state = promptStates[key] || {};
                        const isExpanded = expandedPrompts[key];
                        return (
                          <div
                            key={key}
                            className={`prompt-card ${state.status || ""}`}
                            style={{
                              borderColor: state.status === "done" ? "rgba(62,207,110,.4)"
                                : state.status === "running" ? "rgba(255,106,0,.5)"
                                : state.status === "error" ? "rgba(240,80,80,.4)"
                                : undefined
                            }}
                          >
                            {/* Header */}
                            <div className="prompt-card-header">
                              <div className="prompt-card-angle">{item.angle || item.title}</div>
                              {item.demo && <div className="prompt-card-demo">{item.demo}</div>}
                              {!running && (
                                <button
                                  className="prompt-card-remove"
                                  onClick={(e) => { e.stopPropagation(); removePrompt(item.si, item.pi); }}
                                  title="Remove this prompt"
                                >✕</button>
                              )}
                              {/* Status indicator */}
                              {state.status === "running" && (
                                <div style={{ position: "absolute", top: 8, right: 8, display: "flex", alignItems: "center", gap: 4, fontSize: 10, color: "#ff8c40" }}>
                                  <span className="spin" style={{ borderTopColor: "#ff8c40", width: 11, height: 11 }} />
                                  {Math.round((state.progress || 0) * 100)}%
                                </div>
                              )}
                              {state.status === "done" && (
                                <div style={{ position: "absolute", top: 10, right: 10, fontSize: 13, color: "var(--green)" }}>✓</div>
                              )}
                              {state.status === "error" && (
                                <div style={{ position: "absolute", top: 10, right: 10, fontSize: 13, color: "var(--red)" }}>✗</div>
                              )}
                            </div>
                            ...
                            <div
                              ...
                              onClick={() => setExpandedPrompts(e => ({ ...e, [key]: !e[key] }))}
                            >
                              <div
                                className={`prompt-card-text ${isExpanded ? "" : "clamped"}`}
                              >{item.prompt}</div>
                            </div>

                            {/* Footer */}
                            <div className="prompt-card-footer">
                              <span className="prompt-card-num">#{idx + 1}</span>
                              <button
                                className="prompt-expand-btn"
                                onClick={() => setExpandedPrompts(e => ({ ...e, [key]: !e[key] }))}
                              >{isExpanded ? "Collapse ▲" : "Read full ▼"}</button>
                            </div>
                          </div>
                        );
                      })}
